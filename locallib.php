...
class star_checklist
{
                                                        ...
                                                        $blindmarking)
    {
        ...
        // need formid and userid
        $info->url = $CFG->wwwroot . '/local/star_checklist/index.php?userid=' . $userid;
        ...
        $eventdata->modulename = 'star_checklist';
...
//        $eventdata->contexturl      = $info->url;
                                                               ...
                                                               $assignmentname)
    {
        ...
        $posthtml = '<p><font face="sans-serif">' .
            '<a href="' . $CFG->wwwroot . '/course/view.php?id=' . $course->id . '">STAR Toolkit</a></font></p>';
    ...
    protected function notify_graders(stdClass $submission, $trainer)
    {
        ...
        $formdata = $DB->get_record('local_starchecklist', array('userid' => $user->id));
    ...
    public function send_notification($submission, $userfrom, $userto, $messagetype, $eventtype, $updatetime)
    {
        ...
        $form = $DB->get_record('local_starchecklist', array('id' => $submission->id));
        $modulename = 'STAR Introduction checklist';
    ...
    protected function get_notifiable_users($userid, $trainer)
    {
        ...
        $notifiableparticipants = $notifiabletrainers = array();

        if($trainer) {
            foreach ($potentialusers as $potentialuser) {
                if ($potentialuser->id == $userid) {
                    $notifiableparticipants[$potentialuser->id] = $potentialuser;
                }
            }
            return $notifiableparticipants;
        } else {
            foreach ($potentialusers as $potentialuser) {
                if ($potentialuser->id == $userid) {
                    // Do not send self.
